...
import { 
  Clock, FileText, Image as ImageIcon, Film, File, CheckCheck, 
  AlertTriangle, Bell, User, Hash, MoreVertical, Filter, 
  Search, Calendar, ChevronDown, LayoutList, LayoutGrid,
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
             {message.sender.avatarUrl ? (
                <img src={message.sender.avatarUrl} alt="" className="w-10 h-10 rounded-full object-cover border border-slate-100" />
             ) : (
          ...
          <div>
            <h4 className="text-sm font-bold text-slate-900 leading-tight">{message.sender.fullName}</h4>
...
const MessageFeed: React.FC<MessageFeedProps> = ({ channelId, className }) => {
  ...
  const loadDemoData = () => {
    const demoMessages: Message[] = [
      {
        id: '1',
        content: '🚨 ALERTA SÍSMICA: Se ha detectado movimiento telúrico en la costa. Activar protocolos de evacuación zona B inmediatamente.',
        createdAt: new Date().toISOString(),
        sender: { id: 's1', fullName: 'Sistema de Alerta', username: 'alert', avatarUrl: undefined },
      ...
      {
        id: '2',
        content: 'Reporte de estado de las unidades de transporte. Todas las unidades se encuentran en ruta sin novedades, excepto la unidad 404 que requiere mantenimiento.',
        createdAt: new Date(Date.now() - 1000 * 60 * 30).toISOString(), // 30 mins ago
        sender: { id: 's2', fullName: 'Carlos Operaciones', username: 'carlos', avatarUrl: undefined },
        priority: MessagePriority.MEDIUM,
        isEmergency: false,
        channelId: 'c1',
        attachments: [{ id: 'a1', url: '#', name: 'reporte_diario.pdf', size: 1024 * 500, type: 'application/pdf' }],
        senderId: 's2',
        categoryId: 'cat2',
        isImmediate: false,
      ...
      {
        id: '3',
        content: 'Reunión de coordinación programada para mañana a las 09:00 AM en la sala principal.',
      ...
      },
      {
        ...
        isEmergency: false,
        channelId: 'c1',
        attachments: [{ id: 'a2', url: '#', name: 'evidencia_01.jpg', size: 1024 * 2000, type: 'image/jpeg' }],
        senderId: 's4',
        categoryId: 'cat2',
        isImmediate: false,
        deliveryMethod: DeliveryMethod.PUSH,
  ...
  useEffect(() => {
    ...
    setLoading(true);
    api.getChannelMessages(channelId, 1, 50)
      .then(res => {
  ...
  if (!channelId) {
    return (
      <div className={`flex flex-col items-center justify-center h-full p-8 text-center text-slate-400 bg-slate-50/30 rounded-xl border border-dashed border-slate-200 ${className}`}>
        ...
        <p className="text-sm max-w-xs mx-auto">Elige un canal del menú para ver el historial de mensajes, alertas y comunicados.</p>
      </div>
    );
  }
  ...
  if (loading) {
    return (
      <div className={`flex flex-col items-center justify-center h-full gap-3 ${className}`}>
        ...
        <span className="text-sm font-medium text-indigo-600 animate-pulse">Cargando mensajes...</span>
      </div>
    );
  }

  return (
    <div className={`flex flex-col bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden h-full ${className}`}>
      {/* Header / Toolbar */}
      ...
      {/* Content Feed */}
      <div className="flex-1 overflow-y-auto p-4 sm:p-6 scroll-smooth bg-slate-50/30" ref={scrollRef}>
        {Object.entries(groupedMessages).map(([date, msgs]) => (
          <div key={date} className="mb-8 last:mb-0 animate-in fade-in slide-in-from-bottom-2 duration-500">
            ...
            {/* Messages Container */}
            <div className={viewMode === 'cards' ? 'grid grid-cols-1 md:grid-cols-2 gap-4' : 'flex flex-col gap-0'}>
              {msgs.map((msg) => (
        ...
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 opacity-0 animate-in fade-in duration-700 fill-mode-forwards" style={{ animationDelay: '200ms' }}>
            ...
            <p className="text-slate-400 text-sm mt-1 mb-6">No hay mensajes en este canal todavía.</p>
