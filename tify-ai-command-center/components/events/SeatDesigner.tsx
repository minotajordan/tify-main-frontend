...
export default function SeatDesigner({ event, onUpdate, onSaveStart }: SeatDesignerProps) {
  ...
  const [selectedSeatType, setSelectedSeatType] = useState<'REGULAR' | 'VIP' | 'ACCESSIBLE'>(
    'REGULAR'
  );
  ...
  const handleAddIndividualSeat = (x: number, y: number) => {
    ...
    if (!isFreeForm) {
      ...
      const convert = confirm(
        'Esta zona es de tipo Cuadrícula o General. ¿Deseas convertirla a "Ubicación Libre" para añadir sillas manualmente?'
      );
      if (convert) {
        updateZone(zone.id, {
          rows: 0,
  ...
  const updateZone = (id: string, updates: Partial<EventZone>) => {
    const updatedZones = zones.map((z) => {
      ...
      const numberingChanged =
        updates.continuousNumbering !== undefined ||
      ...
      if (dimsChanged || numberingChanged) {
        ...
        for (let r = 0; r < newZone.rows; r++) {
          for (let c = 0; c < newZone.cols; c++) {
            ...
            const existingSeat = currentZoneSeats.find(
              (s) => (s.gridRow === r && s.gridCol === c) || s.id === legacyId
            );
  ...
  const recalculateSeatLabels = (
    seatsToProcess: EventSeat[],
    zone: EventZone
  ): Map<string, string> => {
  ...
  const handleDuplicateZone = () => {
    ...
    const newSeats = zoneSeats.map((s) => ({
      ...s,
      id: s.id
        .replace(selectedZoneId, newZoneId)
        .replace(s.id.split('-').pop()!, Math.random().toString(36).substr(2, 5)), // Ensure unique ID
      zoneId: newZoneId,
    ...
    <div className="flex h-full border-t border-gray-200">
          ...
          <div className="space-y-4">
                  ...
                  onChange={(e) => {
                    ...
                    if (newType === 'INFO' || newType === 'STAGE') {
                      ...
                      if (hasSeats) {
                        if (
                          !confirm(
                            'Esta zona tiene sillas asignadas. Al cambiar a este tipo, las sillas no estarán disponibles para la venta. ¿Deseas continuar?'
                          )
                        ) {
                          return;
                        }
            ...
            <div className="mb-4">
              ...
              <div className="flex gap-1 items-center h-[38px]">
                {['#4F46E5', '#DC2626', '#16A34A', '#D97706', '#9333EA', '#64748B', '#1F2937'].map(
                  (c) => (
                    <button
                      key={c}
            ...
            <div className="grid grid-cols-2 gap-3 bg-gray-50 p-3 rounded-lg border border-gray-200">
              <div className="col-span-2 text-xs font-bold text-gray-500 mb-1">
              ...
              <div className="col-span-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateZone(selectedZone.id, { rotation: parseInt(e.target.value) || 0 })
                    }
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        onChange={(e) =>
                          updateZone(selectedZone.id, { seatGapX: parseInt(e.target.value) || 0 })
                        }
                        className="flex-1 h-1 bg-gray-200 rounded-lg appearance-none cursor-pointer"
                      />
                      <span className="text-xs text-gray-600 w-8 text-right">
                        {selectedZone.seatGapX ?? selectedZone.seatGap ?? 4}px
                      </span>
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        onChange={(e) =>
                          updateZone(selectedZone.id, { seatGapY: parseInt(e.target.value) || 0 })
                        }
                        className="flex-1 h-1 bg-gray-200 rounded-lg appearance-none cursor-pointer"
                      />
                      <span className="text-xs text-gray-600 w-8 text-right">
                        {selectedZone.seatGapY ?? selectedZone.seatGap ?? 4}px
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (seats.some((s) => s.zoneId === selectedZone.id)) {
                          ...
                          if (
                            !confirm(
                              'Cambiar las filas regenerará la cuadrícula y reiniciará el estado de las sillas. ¿Continuar?'
                            )
                          )
                            return;
                        }
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (seats.some((s) => s.zoneId === selectedZone.id)) {
                          if (
                            !confirm(
                              'Cambiar las columnas regenerará la cuadrícula y reiniciará el estado de las sillas. ¿Continuar?'
                            )
                          )
                            return;
                        }
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateZone(selectedZone.id, { capacity: parseInt(e.target.value) || 0 })
                      }
                      className="w-full px-3 py-2 border border-gray-300 rounded text-sm"
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateZone(selectedZone.id, { price: parseFloat(e.target.value) || 0 })
                      }
      ...
      <div className="flex-1 bg-gray-100 relative overflow-hidden flex flex-col">
        ...
        <div className="absolute top-4 left-4 z-10 bg-white p-2 rounded-lg shadow-sm border border-gray-200 flex gap-2">
          ...
          <span className="flex items-center text-xs w-12 justify-center">
              ...
              return (
                <motion.div
                    ...
                    zones.forEach((other) => {
                      ...
                      vPoints.forEach((vp) => {
                        if (myVPoints.some((mp) => Math.abs(vp - mp) < THRESHOLD)) {
                          if (
                            !newGuides.some(
                              (g) => g.type === 'vertical' && Math.abs(g.pos - vp) < 1
                            )
                          ) {
                      ...
                      hPoints.forEach((hp) => {
                        if (myHPoints.some((mp) => Math.abs(hp - mp) < THRESHOLD)) {
                          if (
                            !newGuides.some(
                              (g) => g.type === 'horizontal' && Math.abs(g.pos - hp) < 1
                            )
                          ) {
                  ...
                  onDragEnd={(_, info) => {
                    ...
                    const newX = currentLayout.x + info.offset.x;
                    const newY = currentLayout.y + info.offset.y;
                          ...
                          <div className="relative w-full h-full overflow-hidden">
                            {Array.from({ length: zone.rows }).map((_, r) =>
                              Array.from({ length: zone.cols }).map((_, c) => {
                                ...
                                const start = zone.startNumber ?? 1;
                                const colNum =
                                  dir === 'LTR' ? c + start : zone.cols - c - 1 + start;
                ...
                <div className="px-6 py-4 border-b flex justify-between items-center bg-white shadow-sm">
                  <div>
                    <h3 className="font-bold text-xl text-gray-900">
                      Gestionar Sillas: {zone.name}
                    </h3>
                    <p className="text-sm text-gray-500">
                      Configura la distribución de sillas para esta zona
                    </p>
                  </div>
                      ...
                      <div className="bg-white p-4 rounded-lg border border-indigo-200 shadow-sm">
                        <div className="flex justify-between items-center mb-4">
                          <h4 className="font-bold text-sm text-gray-900">
                            {selectedSeatIds.length} Sillas Seleccionadas
                          </h4>
                          <button
                            ...
                            {showDeleteConfirm ? (
                              <div className="space-y-2 p-2 bg-red-50 rounded border border-red-100">
                                <p className="text-xs text-red-800 font-medium mb-2">
                                  ¿Cómo deseas eliminar {selectedSeatIds.length} sillas?
                                </p>
                                <button
                                  onClick={() => {
                                ...
                                <button
                                  onClick={() => {
                                    ...
                                    const seatsToDelete = seats.filter((s) =>
                                      selectedSeatIds.includes(s.id)
                                    );
                                    const remainingSeats = seats.filter(
                                      (s) => !selectedSeatIds.includes(s.id)
                                    );
                                    const affectedRows = Array.from(
                                      new Set(seatsToDelete.map((s) => `${s.zoneId}:${s.rowLabel}`))
                                    );
                                    const updates = new Map<string, string>();
                                    ...
                                    affectedRows.forEach((rowKey) => {
                                      ...
                                      const rowSeats = remainingSeats.filter(
                                        (s) => s.zoneId === zId && s.rowLabel === rLabel
                                      );

                                      const areNumeric = rowSeats.every(
                                        (s) => !isNaN(parseInt(s.colLabel))
                                      );
                                      if (areNumeric) {
                                        const sorted = [...rowSeats].sort(
                                          (a, b) => parseInt(a.colLabel) - parseInt(b.colLabel)
                                        );
                                    ...
                                    setSeats(
                                      remainingSeats.map((s) =>
                                        updates.has(s.id)
                                          ? { ...s, colLabel: updates.get(s.id)! }
                                          : s
                                      )
                                    );
                                    setSelectedSeatIds([]);
                                ...
                                <button
                                  onClick={() => {
                                    ...
                                    const seatsToDelete = currentSeats.filter((s) =>
                                      selectedSeatIds.includes(s.id)
                                    );
                                    const remainingSeats = currentSeats.filter(
                                      (s) => !selectedSeatIds.includes(s.id)
                                    );

                                    const affectedZoneIds = Array.from(
                                      new Set(seatsToDelete.map((s) => s.zoneId))
                                    );
                                    const updates = new Map<string, string>();
                                    ...
                                    affectedZoneIds.forEach((zId) => {
                                      const zone = zones.find((z) => z.id === zId);
                                      if (zone) {
                                        const zoneUpdates = recalculateSeatLabels(
                                          remainingSeats.filter((s) => s.zoneId === zId),
                                          zone
                                        );
                      ...
                      <div className="bg-white p-4 rounded-lg border border-indigo-200 shadow-sm">
                        ...
                        <div className="space-y-3">
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                setSeats(
                                  seats.map((s) =>
                                    s.id === selectedSeatId ? { ...s, rowLabel: e.target.value } : s
                                  )
                                );
                              }}
                              className="w-full text-sm border-gray-300 rounded px-2 py-1"
                          ...
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                setSeats(
                                  seats.map((s) =>
                                    s.id === selectedSeatId ? { ...s, colLabel: e.target.value } : s
                                  )
                                );
                              }}
                              className="w-full text-sm border-gray-300 rounded px-2 py-1"
                          ...
                          <div>
                            ...
                            <select
                              value={
                                seats.find((s) => s.id === selectedSeatId)?.status || 'AVAILABLE'
                              }
                              onChange={(e) => {
                                setSeats(
                                  seats.map((s) =>
                      ...
                      <>
                        ...
                        <div className="mb-6">
                          ...
                          <div className="bg-white p-3 rounded-lg border border-gray-200 space-y-3">
                            <div className="mb-3">
                              ...
                              <select
                                ...
                                value={(() => {
                                  ...
                                  if (m === 'ROW' && !s && dH === 'LTR' && dV === 'TTB')
                                    return 'ROW_STD';
                                  if (m === 'ROW' && s && dH === 'LTR' && dV === 'TTB')
                                    return 'ROW_SNAKE';
                                  if (m === 'COLUMN' && !s && dH === 'LTR' && dV === 'TTB')
                                    return 'COL_STD';
                                  if (m === 'COLUMN' && s && dH === 'LTR' && dV === 'TTB')
                                    return 'COL_SNAKE';
                                  return '';
                              ...
                              >
                                ...
                                <option value="ROW_SNAKE">
                                  Horizontal Zig-Zag (1,2,3 &lt;- 6,5,4)
                                </option>
                                <option value="COL_STD">Vertical Estándar (1,4... 2,5...)</option>
                                <option value="COL_SNAKE">
                                  Vertical Zig-Zag (1,6... ^ 7,8...)
                                </option>
                              </select>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              <div>
                                ...
                                <input
                                  ...
                                  value={zone.rows}
                                  onChange={(e) =>
                                    updateZone(zone.id, { rows: parseInt(e.target.value) || 0 })
                                  }
                              ...
                              <div>
                                ...
                                <input
                                  ...
                                  value={zone.cols}
                                  onChange={(e) =>
                                    updateZone(zone.id, { cols: parseInt(e.target.value) || 0 })
                                  }
                                  className="w-full text-sm border-gray-300 rounded"
                                />
                              </div>
                            ...
                            <div className="grid grid-cols-2 gap-2 mt-2">
                              <div>
                                ...
                                <input
                                  ...
                                  onChange={(e) =>
                                    updateZone(zone.id, { seatGapX: parseInt(e.target.value) || 0 })
                                  }
                                  className="w-full text-sm border-gray-300 rounded"
                                />
                              </div>
                              <div>
                                ...
                                <input
                                  ...
                                  onChange={(e) =>
                                    updateZone(zone.id, { seatGapY: parseInt(e.target.value) || 0 })
                                  }
                                  className="w-full text-sm border-gray-300 rounded"
                                />
                              </div>
                            ...
                            <div className="grid grid-cols-2 gap-2 mt-2">
                              <div>
                                ...
                                <select
                                  ...
                                  onChange={(e) =>
                                    updateZone(zone.id, {
                                      numberingDirection: e.target.value as 'LTR' | 'RTL',
                                    })
                                  }
                                >
                              ...
                              <div>
                                ...
                                <select
                                  ...
                                  onChange={(e) =>
                                    updateZone(zone.id, {
                                      verticalDirection: e.target.value as 'TTB' | 'BTT',
                                    })
                                  }
                                >
                              ...
                              <div>
                                ...
                                <select
                                  ...
                                  onChange={(e) =>
                                    updateZone(zone.id, {
                                      numberingMode: e.target.value as 'ROW' | 'COLUMN',
                                    })
                                  }
                                >
                              ...
                              <div className="col-span-2 flex items-center">
                                <input
                                  ...
                                  onChange={(e) =>
                                    updateZone(zone.id, { continuousNumbering: e.target.checked })
                                  }
                                  className="mr-2 h-3 w-3 text-indigo-600 rounded border-gray-300 focus:ring-indigo-500"
                                />
                                <label
                                  htmlFor="continuousNumbering"
                                  className="text-xs text-gray-600"
                                >
                                  No repetir serie (Numeración continua)
                                </label>
                              </div>
                              <div className="col-span-2 flex items-center">
                                <input
                                  ...
                                  onChange={(e) =>
                                    updateZone(zone.id, { numberingSnake: e.target.checked })
                                  }
                                  className="mr-2 h-3 w-3 text-indigo-600 rounded border-gray-300 focus:ring-indigo-500"
                                />
                                <label htmlFor="numberingSnake" className="text-xs text-gray-600">
                                  Numeración en Zig-Zag (Snake)
                                </label>
                              </div>
                              <div>
                                <label className="text-xs text-gray-600 block mb-1">
                                ...
                                <select
                                  ...
                                  onChange={(e) =>
                                    updateZone(zone.id, {
                                      rowLabelType: e.target.value as 'Alpha' | 'Numeric' | 'Roman',
                                    })
                                  }
                                >
                            ...
                            <div className="mt-3">
                              <label className="text-xs text-gray-600 block mb-1">
                                Rotación del Bloque: {zone.rotation || 0}°
                              </label>
                              <input
                                type="range"
                                min="0"
                                ...
                                onChange={(e) =>
                                  updateZone(zone.id, { rotation: parseInt(e.target.value) })
                                }
                                ...
                                for (let r = 0; r < zone.rows; r++) {
                                  for (let c = 0; c < zone.cols; c++) {
                                    ...
                                    if (labelType === 'Numeric') {
                                      ...
                                      rowLabel =
                                        romans[rIdxForLabel] || (rIdxForLabel + 1).toString();
                                    } else {
                                      rowLabel = String.fromCharCode(65 + rIdxForLabel);
                                    }
                            ...
                            <button
                              onClick={() => {
                                if (
                                  !confirm(
                                    'Esto renumerará todas las sillas de la zona según la configuración. ¿Continuar?'
                                  )
                                )
                                  return;
                                ...
                                const TOLERANCE = 5;
                                const ys = Array.from(
                                  new Set(zoneSeats.map((s) => Math.round((s.y || 0) / 10) * 10))
                                ).sort((a, b) => a - b);
                                const xs = Array.from(
                                  new Set(zoneSeats.map((s) => Math.round((s.x || 0) / 10) * 10))
                                ).sort((a, b) => a - b);
                        ...
                        <div>
                          ...
                          <div className="bg-white p-3 rounded-lg border border-gray-200">
                            <p className="text-xs text-gray-600 mb-3">
                              Haz clic en el área de diseño para añadir sillas individualmente.
                              Arrástralas para moverlas. Haz clic en una silla para editarla.
                            </p>
                            <button
                              onClick={() => {
                                ...
                                if (
                                  confirm(
                                    '¿Cambiar a modo detallado? Esto fijará las posiciones actuales y permitirá mover sillas individualmente.'
                                  )
                                ) {
                                  const zoneSeats = seats.filter((s) => s.zoneId === zone.id);
                                    ...
                                    for (let r = 0; r < zone.rows; r++) {
                                      for (let c = 0; c < zone.cols; c++) {
                                        ...
                                        const seat = zoneSeats.find(
                                          (s) => s.rowLabel === rowLabel && s.colLabel === colLabel
                                        );
                  ...
                  <div
                    ...
                    onMouseDown={(e) => {
                      ...
                      if (
                        ...
                        (e.target as HTMLElement).id === 'canvas-content'
                      ) {
                        setIsDraggingCanvas(true);
                        dragStartRef.current = { x: e.clientX - pan.x, y: e.clientY - pan.y };
                      }
                  ...
                  >
                    ...
                    <div
                      ...
                      style={{
                        ...
                        width:
                          ((zones.find((z) => z.id === selectedZoneId) || zones[0])?.layout
                            ?.width || 800) * manualScale,
                        height:
                          ((zones.find((z) => z.id === selectedZoneId) || zones[0])?.layout
                            ?.height || 600) * manualScale,
                      }}
                    >
                      <div
                        ...
                        onClick={(e) => {
                          ...
                          if (zone.rows === 0 && zone.cols === 0) {
                            ...
                            const newSeat: EventSeat = {
                              ...
                              rowLabel: 'F',
                              colLabel: (
                                seats.filter((s) => s.zoneId === zone.id).length + 1
                              ).toString(),
                              status: SeatStatus.AVAILABLE,
                              type: 'REGULAR',
                              x,
                            ...
                            .map((seat) => (
                              <motion.div
                                ...
                                onDrag={(_, info) => {
                                  if (selectedSeatIds.includes(seat.id)) {
                                    setBulkDragOffset({
                                      x: info.offset.x / manualScale,
                                      y: info.offset.y / manualScale,
                                    });
                                  }
                                }}
                                ...
                                onDragEnd={(_, info) => {
                                  ...
                                  if (selectedSeatIds.includes(seat.id)) {
                                    ...
                                    // Single Move
                                    const newX = seat.x + info.offset.x / manualScale;
                                    const newY = seat.y + info.offset.y / manualScale;
                                ...
                                style={{
                                  ...
                                  position:
                                    zone.rows > 0 && zone.cols > 0 ? 'relative' : 'absolute',
                                  transform:
                        ...
                        {zone.rows > 0 && zone.cols > 0 && (
                          <div className="relative w-full h-full bg-white">
                            {Array.from({ length: zone.rows * zone.cols }).map((_, index) => {
                              ...
                              const start = zone.startNumber ?? 1;
                              const colNum =
                                dir === 'LTR' ? col + start : zone.cols - col - 1 + start;
                              const colLabel = colNum.toString();
                              ...
                              const seat = seats.find((s) => {
                                if (s.gridRow !== undefined && s.gridCol !== undefined) {
                                  return (
                                    s.zoneId === zone.id && s.gridRow === row && s.gridCol === col
                                  );
                                }
                                return (
                                  s.zoneId === zone.id &&
                                ...
                                return (
                                  <div
                                    ...
                                    onClick={(e) => {
                                      ...
                                      const rowSeats = currentSeats.filter(
                                        (s) => s.zoneId === zone.id && s.rowLabel === rowLabel
                                      );
                                        ...
                                        if (s.gridCol === undefined) {
                                          ...
                                          if (!isNaN(numericLabel)) {
                                            s.gridCol =
                                              dir === 'LTR'
                                                ? numericLabel - start
                                                : zone.cols - 1 - (numericLabel - start);
