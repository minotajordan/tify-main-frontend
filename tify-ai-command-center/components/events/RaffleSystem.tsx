...
export default function RaffleSystem({ eventId }: RaffleSystemProps) {
  ...
  useEffect(() => {
    ...
    if (filtered.length > 0) {
      ...
      setRouletteItems(
        Array(10)
          .fill(null)
          .map(() => filtered[Math.floor(Math.random() * filtered.length)])
      );
    } else {
      setRouletteItems([]);
    }
  ...
  const handleSpin = async () => {
    ...
    await controls.start({
      ...
      transition: {
        duration: 8,
        ease: [0.05, 0.9, 0.3, 1], // More aggressive spin curve
      },
    });
    ...
    <div className="h-full flex flex-col lg:flex-row gap-6 p-4 lg:p-6 bg-gray-50 overflow-y-auto lg:overflow-hidden">
        ...
        <div>
          ...
          <div className="space-y-1 max-h-[300px] overflow-y-auto pr-1">
            {filteredZones.map((zone) => (
              <label
                key={zone.id}
                className="flex items-center gap-2 p-2 rounded-lg hover:bg-gray-50 border border-gray-100 cursor-pointer"
              >
                <input
                  type="checkbox"
            ...
            {filteredZones.length === 0 && (
              <p className="text-sm text-gray-400 p-2">No se encontraron zonas</p>
            )}
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col items-center bg-gray-900 rounded-xl shadow-inner relative overflow-hidden p-4 min-h-[500px] lg:min-h-0 order-1 lg:order-2">
        ...
        <div className="w-full max-w-5xl relative z-10">
          ...
          >
            <motion.div animate={controls} className="flex gap-4 pl-[50%]" style={{ x: 0 }}>
        ...
        <div className="h-32 mt-8 flex items-center justify-center relative z-10">
          {winner && !isSpinning && (
            ...
            >
              <div className="text-xs font-black uppercase tracking-[0.2em] mb-1 opacity-75">
          ...
          {!winner && !isSpinning && (
            <div className="text-gray-500 text-sm">Presiona GIRAR para comenzar el sorteo</div>
          )}
        </div>
      </div>
    </div>
