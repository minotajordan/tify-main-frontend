...
type SubscriptionItem = {
  id: string;
  channel: { id: string; title: string; icon?: string; logoUrl?: string };
};
type ApproverItem = {
  id: string;
...
const UsersModule: React.FC = () => {
  ...
  const [detailTab, setDetailTab] = useState<
    'overview' | 'subscriptions' | 'assignments' | 'pending' | 'audit' | 'requests'
  >('subscriptions');
  ...
  const [colWidths, setColWidths] = useState<{
    ...
    response: number;
    fecha: number;
  }>({ endpoint: 320, status: 80, type: 90, payload: 300, response: 300, fecha: 140 });
  const setWidth = (key: keyof typeof colWidths, value: number) =>
    setColWidths((prev) => ({ ...prev, [key]: Math.max(80, Math.min(1000, value)) }));
  ...
  const startDrag = (key: keyof typeof colWidths, e: React.MouseEvent) => {
    setDragKey(key);
    setDragStartX(e.clientX);
    setDragStartWidth(colWidths[key]);
  };
  const autofit = (key: keyof typeof colWidths) => {
    ...
    const getText = (row: any) => {
      ...
      if (key === 'payload') {
        try {
          const obj = typeof row.payload === 'string' ? JSON.parse(row.payload) : row.payload;
      ...
      if (key === 'response') {
        try {
          const obj = typeof row.response === 'string' ? JSON.parse(row.response) : row.response;
          return JSON.stringify(obj || null);
        } catch {
  ...
  useEffect(() => {
    const onMove = (e: MouseEvent) => {
      ...
      setWidth(dragKey, dragStartWidth + dx);
    };
    const onUp = () => {
      setDragKey(null);
    };
  ...
  useEffect(() => {
    const key = selected?.id
      ? `tify_user_requests_colwidths_${selected.id}`
      : 'tify_user_requests_colwidths_default';
    try {
      const saved = localStorage.getItem(key);
      if (saved) {
        const obj = JSON.parse(saved);
        if (obj && typeof obj === 'object') setColWidths((prev) => ({ ...prev, ...obj }));
      }
    } catch {}
  }, [selected?.id]);
  useEffect(() => {
  ...
  const colorizeJsonString = (s: string): any => {
    ...
    while ((m = regex.exec(s)) !== null) {
      if (m.index > lastIndex)
        parts.push(
          <span key={lastIndex} className="text-gray-800">
            {s.slice(lastIndex, m.index)}
          </span>
        );
      const token = m[0];
  ...
  useEffect(() => {
    ...
    es.onmessage = (ev) => {
      try {
        ...
        setRequestLog((prev) => [evt, ...prev].slice(0, 500));
      } catch {}
    };
    es.onerror = () => {
      /* silently ignore */
    ...
    if (selected) {
      if (typeof window !== 'undefined') {
        window.dispatchEvent(
          new CustomEvent('tify_breadcrumbs', {
            detail: [
              { label: t('nav.users'), view: 'users' },
              { label: selected.fullName || selected.username || selected.id },
            ],
          })
        );
      }
      ...
      if (typeof window !== 'undefined') {
        window.dispatchEvent(
          new CustomEvent('tify_breadcrumbs', {
            detail: [{ label: t('nav.users'), view: 'users' }],
          })
        );
      }
    }
                    ...
                    >
                      ...
                      <div className="text-right">
                        ...
                        <div className="text-xs font-semibold">
                          {(u as any).subscribedChannelsCount ?? 0}
                        </div>
                      </div>
                    </button>
                  ))}
              ...
              <div className="px-4 py-2 border-b bg-white sticky top-0 z-10 flex items-center gap-3 overflow-x-auto whitespace-nowrap">
                <div className="flex items-center gap-2 text-xs">
                  <button
                    ...
                    className={`px-2 py-1 rounded flex items-center gap-1 ${detailTab === 'subscriptions' ? 'bg-indigo-600 text-white' : 'hover:bg-gray-100'}`}
                    title="Suscripciones"
                  ...
                  <button
                    ...
                    className={`px-2 py-1 rounded flex items-center gap-1 ${detailTab === 'assignments' ? 'bg-indigo-600 text-white' : 'hover:bg-gray-100'}`}
                    title="Asignaciones"
                  ...
                  <button
                    ...
                    className={`px-2 py-1 rounded flex items-center gap-1 ${detailTab === 'pending' ? 'bg-indigo-600 text-white' : 'hover:bg-gray-100'}`}
                    title="Pendientes"
                  ...
                  <button
                    ...
                    className={`px-2 py-1 rounded flex items-center gap-1 ${detailTab === 'requests' ? 'bg-indigo-600 text-white' : 'hover:bg-gray-100'}`}
                    title="Actividad de peticiones"
                    aria-label="Actividad de peticiones"
                  >
                    <ChevronDown size={14} />
                    <span>Actividad</span>
                  </button>
                  {me?.isAdmin && (
                    <button
                      onClick={() => {
                        setDetailTab('audit');
                        if (selected?.id) loadAuditLogs(selected.id);
                      }}
                      className={`px-2 py-1 rounded flex items-center gap-1 ${detailTab === 'audit' ? 'bg-indigo-600 text-white' : 'hover:bg-gray-100'}`}
                      title="Auditoría"
                                ...
                                <button
                                  onClick={async () => {
                                    ...
                                    if (selected.isDisabled) {
                                      ...
                                      await api.disableUser(selected.id);
                                    }
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
                  <div className="p-3 border rounded-lg">
                    ...
                    <button
                      onClick={async () => {
                        if (!selected) return;
                        const s = await api.getUserSubscriptions(selected.id);
                        setSubs(s);
                        setSubsModalOpen(true);
                      }}
                      className="text-2xl font-bold underline decoration-dotted hover:text-indigo-600"
                      title="Ver y gestionar suscripciones"
                      aria-label="Ver y gestionar suscripciones"
                    >
                      {(selected as any).subscribedChannelsCount ?? 0}
                    </button>
                  </div>
                  <div className="p-3 border rounded-lg">
                  ...
                  <div className="p-3 border rounded-lg">
                    ...
                    <div className="text-2xl font-bold">
                      {(selected as any).ownedChannelsCount ?? 0}
                    </div>
                  </div>
                </div>
              )}
                      ...
                      {(topChannels || []).length === 0 ? (
                        ...
                        (topChannels || []).map((it) => (
                          <div key={it.channel.id} className="flex items-center gap-3">
                            <img
                              src={it.channel.logoUrl || DEFAULT_AVATAR}
                      ...
                      <div className="max-h-[60vh] overflow-y-auto">
                        {subs.length === 0 ? (
                          ...
                          subs.map((s) => (
                            ...
                            >
                              <img
                                src={s.channel.logoUrl || DEFAULT_AVATAR}
                                alt={s.channel.title}
                      ...
                      <div className="flex items-center justify-between px-4 py-3 border-b">
                        ...
                        <div className="flex items-center gap-3">
                          <button
                            ...
                            className="px-2 py-1 border rounded text-xs hover:bg-indigo-50"
                        ...
                        <table className="min-w-full text-xs table-fixed">
                          <thead>
                            <tr className="text-left text-gray-600 sticky top-0 bg-white/90 backdrop-blur border-b shadow-sm">
                              <th
                                ...
                                style={{ width: colWidths.endpoint }}
                              >
                                Endpoint
                                <div
                                  className="absolute top-0 right-0 h-full w-2 cursor-col-resize hover:bg-indigo-100"
                                  onMouseDown={(e) => startDrag('endpoint', e)}
                                  onDoubleClick={() => autofit('endpoint')}
                                />
                              </th>
                              <th
                                className="py-2 px-3 border-b relative select-none"
                              ...
                              >
                                ...
                                <div
                                  ...
                                  onDoubleClick={() => autofit('type')}
                                />
                              </th>
                              <th
                                className="py-2 px-3 border-b relative select-none"
                                style={{ width: colWidths.payload }}
                              >
                                Payload
                                <div
                                  className="absolute top-0 right-0 h-full w-2 cursor-col-resize hover:bg-indigo-100"
                                  onMouseDown={(e) => startDrag('payload', e)}
                                  onDoubleClick={() => autofit('payload')}
                                />
                              </th>
                              <th
                                className="py-2 px-3 border-b relative select-none"
                                style={{ width: colWidths.response }}
                              >
                                Response
                                <div
                                  className="absolute top-0 right-0 h-full w-2 cursor-col-resize hover:bg-indigo-100"
                                  onMouseDown={(e) => startDrag('response', e)}
                                  onDoubleClick={() => autofit('response')}
                                />
                              </th>
                              <th
                                className="py-2 px-3 border-b relative select-none"
                              ...
                              >
                                ...
                                <div
                                  className="absolute top-0 right-0 h-full w-2 cursor-col-resize hover:bg-indigo-100"
                                  onMouseDown={(e) => startDrag('fecha', e)}
                                  onDoubleClick={() => autofit('fecha')}
                                />
                              </th>
                            </tr>
                          </thead>
                          <tbody>
                            {requestLog.map((row, idx) => (
                              <tr
                                key={idx}
                                className="border-b hover:bg-indigo-50/40 even:bg-gray-50"
                              >
                                <td
                                  className="py-2 px-3 font-mono text-[11px] break-all"
                                  style={{ width: colWidths.endpoint }}
                                >
                                  {row.endpoint}
                                </td>
                                <td className="py-2 px-3" style={{ width: colWidths.status }}>
                                  <span
                                    className={`${row.status >= 500 ? 'bg-red-100 text-red-700' : row.status >= 400 ? 'bg-yellow-100 text-yellow-700' : 'bg-green-100 text-green-700'} px-2 py-1 rounded`}
                                  >
                                    {row.status || 0}
                                  </span>
                                </td>
                                <td className="py-2 px-3" style={{ width: colWidths.type }}>
                                  <span className="px-2 py-1 bg-gray-100 text-gray-700 rounded">
                                    {extractMethod(row.endpoint)}
                                  </span>
                                </td>
                                <td
                                  className="py-2 px-3 font-mono text-[11px] break-all max-h-20 overflow-hidden cursor-pointer"
                                  style={{ width: colWidths.payload }}
                                  onClick={async () => {
                                    setJsonModalTitle('Payload');
                                    try {
                                      const full = await fetch(
                                        `${API_BASE}/streams/user-requests/${selected!.id}/events/${row.id}`
                                      ).then((r) => r.json());
                                ...
                                <td
                                  ...
                                  style={{ width: colWidths.fecha }}
                                  title={new Date(row.date).toLocaleString()}
                                >
                                  {formatElapsed(row.date)}
                                </td>
                              </tr>
                            ))}
                            {requestLog.length === 0 && (
          ...
          <div className="bg-white w-full max-w-lg rounded-xl shadow-lg border border-gray-200">
            ...
            <div className="p-6 space-y-3">
              ...
              <div className="pt-2 flex items-center justify-end gap-2">
                ...
                <button
                  onClick={async () => {
                    await api.createUser(createForm as any);
                    const res: any = await api.getUsersPaged(1, usersLimit, query);
                    setUsers(res.items || []);
                    setUsersPage(1);
                    setUsersPages(res.pagination?.pages || 1);
          ...
          <div className="bg-white w-full h-full rounded-none shadow-lg border border-gray-200 flex flex-col">
            <div className="p-4 border-b border-gray-100 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() => {
                    try {
                      navigator.clipboard.writeText(JSON.stringify(jsonModalValue, null, 2));
                    } catch {}
                  }}
                  className="px-3 py-2 border border-gray-300 rounded text-xs"
                >
            ...
            <div className="flex items-center justify-between px-4 py-2 border-b">
              ...
              <div className="flex items-center gap-2 text-xs">
                ...
                <button onClick={() => setExpanded(new Set())} className="px-2 py-1 border rounded">
                  Plegar
                </button>
              </div>
            </div>
            <div className="flex-1 overflow-auto p-4">
              {(() => {
                ...
                try {
                  obj =
                    typeof jsonModalValue === 'string'
                      ? JSON.parse(jsonModalValue)
                      : jsonModalValue;
                } catch {
                  obj = jsonModalValue;
                }
                    ...
                    return (
                      <span>
                        ...
                        <mark className="bg-yellow-200 text-gray-900">
                          {s.slice(idx, idx + jsonSearch.length)}
                        </mark>
                        {s.slice(idx + jsonSearch.length)}
                      </span>
                    );
                  };
                  if (value === null) return <span className="text-orange-600">null</span>;
                  if (t === 'string')
                    return <span className="text-green-700">"{mark(value)}"</span>;
                  if (t === 'number')
                    return <span className="text-purple-700">{String(value)}</span>;
                  if (t === 'boolean')
                    return <span className="text-orange-700">{String(value)}</span>;
                  if (isArr) {
                    return (
                      <div>
                        <button
                          onClick={toggle}
                          className="inline-flex items-center gap-1 text-xs px-1 py-[1px] rounded hover:bg-gray-100 text-gray-600"
                        >
                        ...
                        {open &&
                          entries.map(([k, v]) => (
                            <div key={k} className="pl-4">
                              <span className="text-blue-700">"{mark(k)}"</span>
                              <span>: </span>
                ...
                return (
                  <div className="text-xs leading-5 font-mono">
                    <JsonNode value={obj} />
                  </div>
                );
              })()}
                            ...
                            <div className="rounded-2xl bg-white border border-yellow-200 shadow-lg px-3 py-2">
                              <div className="flex items-center gap-2">
                                <button
                                  onClick={async () => {
                                    if (!selected) return;
                                    await api.removeSubscription(selected.id, s.channel.id);
