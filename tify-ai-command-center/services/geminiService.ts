...
const buildSystemContext = async (): Promise<string> => {
  try {
    ...
    const [channels, stats] = await Promise.all([
        api.getChannels(),
        uid ? api.getUserStats(uid) : Promise.resolve({ subscribedChannelsCount: 0, messagesCount: 0, ownedChannelsCount: 0, pendingApprovalsCount: 0, recentActivity: [] } as any)
    ]);
...
export const generateMessageDraft = async (prompt: string, channelName: string): Promise<string> => {
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      config: {
        systemInstruction: "You are a helpful communication assistant for a corporate messaging app.",
      }
    });
...
export const analyzeMessageSentiment = async (message: string): Promise<{ sentiment: string, score: number }> => {
